if (window.mdEditorRtAppInitialized) {
    ...
    const MdEditor = window.MdEditorRT.MdEditor;
...
// 引入浮窗面板 (直接使用全局变量)
// const OperationsFloatingPanel = window.OperationsFloatingPanel;
// const SimpleCacheUI = window.SimpleCacheUI;
...
// setAutoSaveInterval 函数在下面定义，这里不再重复定义
...
function TimeDisplay() {
    const [currentTime, setCurrentTime] = window.React.useState(getCurrentTime());
...
function SaveButton(props) {
    return window.React.createElement(CustomToolbarButton, {
        title: '手动保存',
        icon: '💾',
...
function ThemeDropdown(props) {
    ...
    const handleChange = window.React.useCallback((value) => {
    ...
    return window.React.createElement(window.MdEditorRT.DropdownToolbar, {
        title: props.title || props.value,
        visible: visible,
        onChange: setVisible,
        disabled: props.disabled,
        overlay: window.React.createElement('ul', { className: 'md-editor-menu', role: 'menu' },
            options.map((option) =>
                window.React.createElement('li', {
        ...
        trigger: window.React.createElement(window.React.Fragment, null,
            window.React.createElement('span', { className: 'md-editor-icon' }, '🎨'), // 使用简单的图标
            props.showToolbarName && window.React.createElement('div', { className: 'md-editor-toolbar-item-name' }, props.title || props.value)
        )
    });
}
...
function App() {
    const [text, setText] = window.React.useState(''); // Initialize with empty string
    const [previewTheme, setPreviewTheme] = window.React.useState('default'); // 初始化预览主题
...
---`;
    ...
    return window.React.createElement(MdEditor, {
            modelValue: text,
            onChange: setText,
        ...
        defToolbars: [
            window.React.createElement(SaveButton, { key: 'save-btn' }),
            window.React.createElement(CleanHexoButton, { key: 'clean-btn' }),
            window.React.createElement(ExportButton, { key: 'export-btn' }),
            window.React.createElement(UploadImageButton, { key: 'upload-btn' }),
            window.React.createElement(ThemeDropdown, { // 添加主题切换按钮
                key: 'theme-switch',
                value: previewTheme,
        ...
        defFooters: [
            window.React.createElement(TimeDisplay, { key: 'time-display' })
        ],
        onSave: (v, h) => {
            // 当用户按 Ctrl+S 时触发手动保存
            window.handSave();
...
// cleanHexo 函数已在上面定义，这里不再重复定义

// loadMdFile 函数已在上面定义，这里不再重复定义
}