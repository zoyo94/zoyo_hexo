...
const { ensureHexoFrontMatter } = require('./utils/markdownParser');
...
// 全局状态跟踪，用于监控长时间运行的任务（提前定义，避免路由找不到函数）
...
// 创建 Multer 实例的通用配置 (改为diskStorage解决内存占用隐患)
...
// 移除自定义 writeFile 函数，直接使用 fs.promises.writeFile
...
// 验证路径安全防范目录遍历（Path Traversal）漏洞
const validateSafePath = (targetPath) => {
    const resolvedPath = path.resolve(targetPath);
    const workDirResolved = path.resolve(CONFIG.WORK_DIR);
    if (resolvedPath !== workDirResolved && !resolvedPath.startsWith(workDirResolved + path.sep)) {
        const err = new Error('非法的文件路径访问（Path Traversal detected）');
        err.status = 403;
        throw err;
    }
    return resolvedPath;
...
/**
 * 根路由：显示美化的接口文档 Dashboard
 */
app.get('/', (req, res) => {
    const apiDocs = [
        { group: '文件操作 (Files)', endpoints: [
            { method: 'GET', path: '/get_md', desc: '获取 Markdown 文件内容', params: 'filename, directory' },
            { method: 'POST', path: '/save_md', desc: '保存 Markdown 及视频附件', params: 'filename, directory, content, video_folder' },
            { method: 'POST', path: '/upload', desc: '单文件上传 (MD)', params: 'file (multipart)' },
            { method: 'GET', path: '/api/list-files', desc: '列出所有已管理的文件', params: '无' }
        ]},
        { group: '资源管理 (Assets)', endpoints: [
            { method: 'POST', path: '/upimg', desc: '图片/视频附件上传', params: 'file, customDestination' },
            { method: 'POST', path: '/upload-folder', desc: '文件夹批量上传', params: 'files (multipart)' },
            { method: 'POST', path: '/move_image', desc: '资源跨目录移动', params: 'fileName, folderName, new_folderName, original_dir' }
        ]},
        { group: '系统工具 (System)', endpoints: [
            { method: 'GET', path: '/directory-tree', desc: '获取资源目录树', params: 'directory (safePath)' },
            { method: 'POST', path: '/create_folder', desc: '创建新文件夹', params: 'folderName, folderPath' },
            { method: 'POST', path: '/clean_hexo', desc: '手动清理 Hexo 缓存', params: '无' },
            { method: 'POST', path: '/tgz_download', desc: '导出压缩打包文件', params: 'directoryName, selectedFile, original_dir, markdownContent' },
            { method: 'GET', path: '/api/save-status', desc: '查询服务器处理状态', params: '无' }
        ]}
    ];
    ...
    <head>
        ...
        <title>MD Editor Backend Dashboard</title>
        ...
        <style>
            ...
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .GET { background: rgba(0, 255, 0, 0.1); color: #00ff00; }
            .POST { background: rgba(0, 242, 254, 0.1); color: var(--primary); }
            .path {
                display: block;
                font-family: 'JetBrains Mono', monospace;
                font-size: 1rem;
            ...
            .desc { font-size: 0.9rem; opacity: 0.7; margin-bottom: 15px; }
            .params {
                font-size: 0.8rem;
            ...
            .params-label { color: var(--secondary); margin-right: 5px; font-weight: 600; }
            footer { text-align: center; margin-top: 50px; opacity: 0.3; font-size: 0.8rem; }
        </style>
    </head>
    <body>
        <div class="container">
            <header>
                <h1>MD Editor Backend Control Center</h1>
                <div class="status-bar">
                    <span class="status-dot"></span>
                    <span>Server Operational on Port ${CONFIG.PORT}</span>
                    <span style="margin: 0 10px; opacity: 0.3;">|</span>
                    <span style="color: var(--secondary)">Node.js ${process.version}</span>
                </div>
            </header>
                        ...
                        <div class="grid">
                            ${group.endpoints.map(api => `
                                <div class="card">
                                    ...
                                    <div class="params">
                                        <span class="params-label">Params:</span> ${api.params}
                                    </div>
                                </div>
                            `).join('')}
                        </div>
            ...
            <footer>
                Designed with ❤️ for MD Editor Integration Project
            </footer>
        </div>
    </body>
    </html>
...
const autoCleanHexo = () => {
    return new Promise((resolve, reject) => {
        ...
        // 增设超时保护机制，防止 Hexo 挂起导致应用“永久锁死”
        const timeoutDuration = 60000; // 60 秒超时限制
        let hexoCleanProcess = null;
        let hexoGenerateProcess = null;
        let isDone = false; // 防止重复拒绝或解析

        const timer = setTimeout(() => {
            if (isDone) return;
            isDone = true;
            logger.error(`🚨 Hexo 构建超时（>${timeoutDuration}ms），强制结束进程以释放锁`);
            if (hexoCleanProcess && !hexoCleanProcess.killed) hexoCleanProcess.kill('SIGKILL');
            if (hexoGenerateProcess && !hexoGenerateProcess.killed) hexoGenerateProcess.kill('SIGKILL');
            isCleaningInProgress = false;
            reject(new Error('Hexo 构建超时挂起，已被强制结束'));
        }, timeoutDuration);
        ...
        // 执行 hexo clean
        hexoCleanProcess = spawn('npx', ['hexo', 'clean'], {
            cwd: CONFIG.HEXO_DIR,
            stdio: 'pipe'
        });

        // 捕获 hexo clean 的输出
        hexoCleanProcess.stdout.on('data', (data) => {
            logger.info(`Hexo Clean stdout: ${data.toString().trim()}`);
        });

        hexoCleanProcess.stderr.on('data', (data) => {
            logger.warn(`Hexo Clean stderr: ${data.toString().trim()}`);
        });

        hexoCleanProcess.on('close', (cleanCode) => {
            if (isDone) return; // 如果已超时，则跳过
            logger.info(`Hexo clean 进程结束，退出码: ${cleanCode}`);

            if (cleanCode !== 0) {
                logger.error(`Hexo clean 失败，退出码: ${cleanCode}`);
                return safeReject(new Error(`Hexo clean failed with code ${cleanCode}`));
            }

            logger.info('Hexo clean 完成，开始执行 Hexo generate...');

            hexoGenerateProcess = spawn('npx', ['hexo', 'generate'], {
                cwd: CONFIG.HEXO_DIR,
                stdio: 'pipe'
            });

            // 捕获 hexo generate 的输出
            hexoGenerateProcess.stdout.on('data', (data) => {
                logger.info(`Hexo Generate stdout: ${data.toString().trim()}`);
            });

            hexoGenerateProcess.stderr.on('data', (data) => {
                logger.warn(`Hexo Generate stderr: ${data.toString().trim()}`);
            });

            hexoGenerateProcess.on('close', (generateCode) => {
                if (isDone) return;
                ...
                if (generateCode !== 0) {
                    ...
                    return safeReject(new Error(`Hexo generate failed with code ${generateCode}`));
                }
            ...
            hexoGenerateProcess.on('error', (error) => {
                logger.error(`Hexo generate 进程错误: ${error.message}`);
                safeReject(error);
            });
        });

        hexoCleanProcess.on('error', (error) => {
...
const errorHandler = (err, req, res, next) => {
    logger.error(`服务器错误: ${err.message}`, err.stack);
    
    // 如果存在 multer 上传的临时文件，在错误发生时尝试清理
    if (req.file) {
        fs.promises.unlink(req.file.path).catch(e => logger.warn(`错误处理中清理临时文件失败: ${e.message}`));
    }
    if (req.files) {
        if (Array.isArray(req.files)) {
            req.files.forEach(file => fs.promises.unlink(file.path).catch(e => logger.warn(`错误处理中批量清理临时文件失败: ${e.message}`)));
        } else {
            Object.values(req.files).flat().forEach(file => fs.promises.unlink(file.path).catch(e => logger.warn(`错误处理中批量清理临时文件失败: ${e.message}`)));
        }
    }

    const message = process.env.NODE_ENV === 'production' ? '服务器内部错误' : err.message;
    res.status(err.status || 500).json({ success: false, message: message });
};
...
app.post('/upload', uploadMd.single('md_file'), async (req, res, next) => {
    try {
        ...
        // 处理文件内容：确保包含 Hexo Front Matter (改为从临时文件读取)
        let fileContent = await fs.promises.readFile(req.file.path, 'utf8');
        fileContent = ensureHexoFrontMatter(fileContent, finalName, logger);

        try {
            // 写入工作目录（由于软链接，文件会自动出现在 _posts 目录）
            await fs.promises.writeFile(filePath, fileContent);
            // 清理临时文件
            await fs.promises.unlink(req.file.path).catch(e => logger.warn(`清理临时文件失败: ${e.message}`));
...
app.post('/upimg', uploadImg.array('image'), async (req, res, next) => {
    try {
        ...
        const targetDir = validateSafePath(path.join(CONFIG.WORK_DIR, customDestination.replace(/^\.\//, '')));
        const imagePaths = [];
        const filenames = [];
        ...
        for (const file of req.files) {
            ...
            const finalName = await FileUtils.generateUniqueFilename(originalName, targetDir); // 异步调用

            const imagePath = path.join(targetDir, finalName);
            await fs.promises.copyFile(file.path, imagePath); // 使用复制而非直接写入buffer
            await fs.promises.unlink(file.path).catch(e => logger.warn(`清理临时文件失败: ${e.message}`)); // 清理临时文件
...
app.post('/upload-folder', uploadImg.array('files'), async (req, res, next) => {
    try {
        ...
        for (const file of req.files) {
            ...
            logger.info(`${originalName} -> ${finalName}`);
            const fullPath = validateSafePath(path.join(CONFIG.WORK_DIR, folderName, finalName));
            ...
            if (finalName.endsWith('.md')) {
                let mdContent = await fs.promises.readFile(file.path, 'utf8');
                mdContent = ensureHexoFrontMatter(mdContent, finalName, logger);
                let fileBuffer = Buffer.from(mdContent, 'utf8');
                ...
                await fs.promises.writeFile(fullPath, fileBuffer); // 写入处理后的MD文件
                logger.info(`MD 文件已保存到工作目录: ${fullPath} (通过软链接同步到 _posts)`);
            } else {
                await fs.promises.copyFile(file.path, fullPath); // 复制非MD文件
            }
            await fs.promises.unlink(file.path).catch(e => logger.warn(`清理临时文件失败: ${e.message}`));
...
app.get('/directory-tree', async (req, res, next) => {
    try {
        ...
        // 安全校验：解析并验证路径
        const safeDir = validateSafePath(path.join(CONFIG.WORK_DIR, directory.replace(/^\.\//, '')));

        // 确保基础目录存在
        try {
            await fs.promises.access(safeDir);
        } catch (error) {
            logger.warn(`请求的目录不存在或无权限访问: ${safeDir}, 错误: ${error.message}`);
            return res.json([]); 
        }

        const children = await readDirectory(safeDir);
        const tree = { name: path.basename(safeDir), children };
...
app.get('/get_md', async (req, res, next) => {
    try {
        ...
        let filePath = validateSafePath(path.join(CONFIG.WORK_DIR, directory.replace(/^\.\//, ''), filename));
        ...
        try {
            ...
            if (!filename.endsWith('.md')) {
                const altPath = validateSafePath(path.join(CONFIG.WORK_DIR, directory.replace(/^\.\//, ''), `${filename}.md`));
                try {
                    await fs.promises.access(altPath);
                    filePath = altPath; // 补全后缀后找到了文件
                } catch (e) {
 ...
 * POST /save_md
 */
app.post('/save_md', async (req, res, next) => {
    try {
        // 设置处理状态为“正在保存MD文件”
        setProcessingStatus(true, 'saving_md');

        const { destination, filename, currentContent, hasImages, oldFilename } = req.body;
        if (!destination || !filename) throw new Error('缺少必要参数');

        const destDir = destination.replace(/^\.\//, '');
        const filePath = validateSafePath(path.join(CONFIG.WORK_DIR, destDir, filename));
        const baseName = path.basename(filename, '.md');
        let finalContent = currentContent || '';

        // 处理重命名逻辑：清理旧文件和文件夹
        if (oldFilename && oldFilename !== 'cache' && oldFilename !== filename) {
            const oldFilePath = validateSafePath(path.join(CONFIG.WORK_DIR, destDir, oldFilename.endsWith('.md') ? oldFilename : `${oldFilename}.md`));
            const oldBaseName = path.basename(oldFilename, '.md');
            const oldImageDir = path.join(path.dirname(oldFilePath), oldBaseName);
            const newImageDir = path.join(path.dirname(filePath), baseName);

            try {
                // 1. 如果旧媒体文件夹存在，则重命名为新文件夹
                try {
                    await fs.promises.access(oldImageDir);
                    // 确保目标父目录存在
                    await fs.promises.mkdir(path.dirname(newImageDir), { recursive: true });
                    await fs.promises.rename(oldImageDir, newImageDir);
                    logger.info(`✅ 媒体文件夹已迁移: ${getRelativePath(oldImageDir)} -> ${getRelativePath(newImageDir)}`);
                } catch (e) {
                    // 旧文件夹不存在，忽略
                }

                // 2. 如果旧 MD 文件存在且不是新文件本身，则删除它
                if (oldFilePath !== filePath) {
                    try {
                        await fs.promises.access(oldFilePath);
                        await fs.promises.unlink(oldFilePath);
                        logger.info(`🗑️ 旧 MD 文件已删除: ${getRelativePath(oldFilePath)}`);
                    } catch (e) {
                        // 旧文件不存在，忽略
                    }
                }
            } catch (err) {
                logger.error(`❌ 重命名清理失败: ${err.message}`);
                // 继续执行保存新文件，不中断流程
            }
        }
        ...
        if (hasImages && finalContent) {
            ...
            if (imageMatches) {
                for (const match of imageMatches) {
                    ...
                    logger.info(`正在处理图片路径: ${imagePath}`);
                    // 如果是相对路径且不在同名文件夹中
                    if (!imagePath.startsWith('http') && !imagePath.startsWith(baseName + '/')) {
                        const sourceImagePath = validateSafePath(path.join(CONFIG.WORK_DIR, destination.replace(/^\.\//, ''), imagePath));
                ...
                for (const match of videoMatches) {
                    ...
                    if (!videoPath.startsWith('http') && !videoPath.startsWith(baseName + '/')) {
                        ...
                        if (!videoPath.includes('/')) {
                            sourceVideoPath = validateSafePath(path.join(CONFIG.WORK_DIR, destination.replace(/^\.\//, ''), 'IMG', videoFileName));
                        } else {
                            // 否则，按原样处理（例如 ./IMG/video.mp4）
                            sourceVideoPath = validateSafePath(path.join(CONFIG.WORK_DIR, destination.replace(/^\.\//, ''), videoPath.replace(/^\.\//, '')));
                        }
...
app.post('/create_folder', noFileUpload.single('file'), async (req, res, next) => {
    try {
        ...
        const fullFolderPath = validateSafePath(path.join(CONFIG.WORK_DIR, folderPath.replace(/^\.\//, ''), folderName));
...
app.post('/move_image', noFileUpload.single('file'), async (req, res, next) => {
    try {
        ...
        const sourcePath = validateSafePath(path.join(CONFIG.WORK_DIR, folderName, fileName));
        const destinationPath = validateSafePath(path.join(CONFIG.WORK_DIR, new_folderName, fileName));
...
app.post('/api/check-file', noFileUpload.single('file'), async (req, res, next) => {
    try {
        ...
        const filePath = validateSafePath(path.join(CONFIG.WORK_DIR, filename));
        let exists = false;
        try {
            await fs.promises.access(filePath);
            exists = true;
...
app.post('/tgz_download', noFileUpload.none(), async (req, res, next) => {
    try {
        ...
        // 检查目录是否存在
        const fullDirPath = validateSafePath(path.join(CONFIG.WORK_DIR, directoryName));
        try {
        ...
        // 检查MD文件是否存在并根据条件写入内容
        if (selectedFile) {
            const mdFilePath = validateSafePath(path.join(CONFIG.WORK_DIR, selectedFile));
